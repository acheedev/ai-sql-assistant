CREATE TABLE t_semantic_object (
    ...
    CONSTRAINT t_semantic_object_chk1 CHECK ( include_in_ai IN ( 'Y',
                                                                 'N' ) ),
    CONSTRAINT t_semantic_object_chk2 CHECK ( preferred_for_ai IN ( 'Y',
                                                                    'N' ) ),
    CONSTRAINT t_semantic_object_chk3
        CHECK ( object_type IN ( 'TABLE',
                                 'VIEW',
                                 'BUSINESS_VIEW' ) ),
    CONSTRAINT t_semantic_object_chk4 CHECK ( status IN ( 'ACTIVE',
                                                          'INACTIVE' ) )
);
